...
Implement the GitHub issue given as the command argument (the issue number, e.g. `/implement-issue 56`) end to end: understand the requirement, plan, get plan approval, implement the full vertical slice, verify against the acceptance criteria, get acceptance approval, record an implementation comment on the issue, and offer to ship it as a pull request.

This prompt orchestrates existing skills instead of re-deriving their mechanics:
...
The issue number is referred to below as `<issue>`.
...
- **Test-authoring mode** — the issue asks only to add or extend Playwright API tests for behaviour that **already exists** (for example the `testing`-labelled framework tasks). No production code is expected. In this mode the tests themselves are the deliverable: skip the vertical-slice wiring and the browser UI walk. If, while writing the tests, you discover the API is actually broken, stop and tell me — that becomes a separate defect, not part of this task.
...
Use the **`github-issue`** skill to read issue `<issue>` (body, comments, parent/sub-issues), confirm it is feature-type by its labels (if labelled `bug`, stop and confirm — that is the defect-fix flow, not this one), and turn the requirement into an explicit, checkable acceptance list. Flag any material gaps to me before planning; do not invent them.
...
- Save the plan as Markdown under `docs/tasks/` (create the folder if needed), named `issue-<issue>-<short-slug>.md`.
...
- Ask me whether to commit the changes and open a pull request targeting `main`. Do not commit, push, or create the PR without my explicit go-ahead.
- If agreed, use the **`open-pr`** skill: commit onto branch `<issue>-<short-slug>` (via the `git-commit` skill), include the `docs/tasks/issue-<issue>-<short-slug>.md` plan in the commit, and open the PR into `main`.
